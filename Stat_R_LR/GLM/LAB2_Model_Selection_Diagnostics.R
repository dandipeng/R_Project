...
birthwt.step <- stepAIC(birthwt.glm, trace = FALSE)
  # default direction is backward when scope is missing
birthwt.step$anova
...
birthwt.step2 <- stepAIC(birthwt.glm, trace = FALSE, scope = Scope)
 # default direction is both when scope is specified
birthwt.step2$anova
...
# no systematic pattern -> next step: Runs Test
